...
if df.empty:
    st.warning(
        "No data found in the database. Please run your data cleaning script first!")
else:
    ...
    if 'year' in df.columns:
        ...
        if len(valid_years) > 1:
            ...
            selected_years = st.sidebar.slider(
                "Select Year Range", min_value=min_yr, max_value=max_yr, value=(min_yr, max_yr))
        elif len(valid_years) == 1:
            st.sidebar.info(
                f"Note: Dataset contains only year {int(valid_years[0])}")
...
"""
    ...
    selected_seasons = st.sidebar.multiselect(
        "Select Seasons",
        options=df['season'].dropna().unique().tolist() if 'season' in df.columns else [],
    ...
    hab_column = next((col for col in ['Habitat', 'habitat', 'HABITAT', 'habitat_type'] if col in df.columns), df.columns[0])
    season_column = next((col for col in ['season', 'Season', 'SEASON', 'season_name'] if col in df.columns), df.columns[0])
    sheet_column = next((col for col in ['sheet_name', 'Sheet_Name', 'shhet'] if col in df.columns),df.columns[0])
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊Overview Charts", "🗓️Temporal & Trends", "🔍 Interactive Data Explorer"])
    with tab1:
        ...
        with col_a:
            if not filtered_df.empty and hab_column in filtered_df.columns:
                ...
                fig_hab = px.bar(
                    ...
                    # Power BI Style comma for formatting
                    hover_data={'Habitat': True, 'Observation Count': ':,'}
                )
                fig_hab.update_traces(
                    hovertemplate="<b>Habitat:</b> %{x}<br><b>Total Observations:</b> %{y:,}<extra></extra>")
...
"""
        ...
        with col_b:
            if not filtered_df.empty and 'season' in filtered_df.columns:
                seas_counts = filtered_df['season'].value_counts(
                ...
                fig_seas.update_traces(
                    hovertemplate="<b>Season:</b> %{label}<br><b>Count:</b> %{Value:,}<br><b>Percentage:</b> %{percent}<extra></extra>")
...
"""
    ...
    with tab2:
        ...
        if not filtered_df.empty and ('year' in df_cols or 'Date' in filtered_df.columns):
            ...
            year_col = 'year' if 'year' in filtered_df.columns else (
                'Year' if 'Year' in filtered_df.columns else 'Date')
            hab_col = 'habitat_type' if 'habitat_type' in filtered_df.columns else 'Location_Type'
            # Group data safely for the line chart
            treand_data = filtered_df.groupby(
                [year_col, hab_col]).size().reset_index(name='Observation Count')
            fig_trend = px.line(
                treand_data,
                x=year_col,
                y='Observation Count',
                ...
                template="plotly_white",
                color_discrete_map={
                    'Forest': '#2d6a4f', 'Grassland': '#d4a373'}
            )
...
"""
            ...
            st.info(
                "No temporal data available for the current filter selection. Please adjust your filters above.")
    with tab3:
        ...
        if filtered_df is None or filtered_df.empty:
            ...
            if search_query:
                mask = display_df.astype(str).apply(
                    lambda x: x.str.contains(search_query, case=False)).any(axis=1)
